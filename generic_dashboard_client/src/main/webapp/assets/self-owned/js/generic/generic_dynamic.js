...
function DASHBOARD_DYNAMIC_TABLE_ENV_SETUP() {
    ko.components.register('matrix_dynamic_table', {
        viewModel: function (params) {
            var self = this;
            //ds:{
...
//                self.set_refresh_interval();
        ...
        template: {
            element: 'matrix_dynamic_table-template'
        }
    });
...
function create_dynamic_chart_pie(ds, destination_div_id) {
    var chart = ChartPOJO.generate_default_chart(destination_div_id, 'pie');
...
function successListener_dynamic_chart() {
    ...
    if (arguments && arguments[1]) {
        ...
        var style = chart.getOption().style;
        if (style == "pie") {
